...
%        'subj14-ji','subj15-ma','subj17-ga','subj18-an','subj19-ni'}; %array
    subjlist = {'subj02-el','subj03-yu','subj05-je','subj07-pr','subj08-bo',...
       'subj09-az','subj10-oc','subj11-lu','subj12-al','subj13-ni',...
       'subj14-ji','subj15-ma','subj17-ga','subj18-an'}; %array
   MaxPlotFix = 6; 
       ...
       % remove those trials with only one fixation
        ...
        if strcmp(type, 'array')
            ...
            for j = 1:length(fx)-1
                ...
                siml = load(filename);
                sim = [sim double(siml.x(LayerSelected))];
            end
            sim = [nan sim];
            if ~isempty(sim)
                sim = sim(1:end);
...
save(['Mat/similarity_' type '_layer_' num2str(LayerSelected) '.mat'],'SIMRETURN','SIMNONRETURN','SIMRATIO');
%%

hb = figure; hold on;
load(['Mat/similarity_' type '_layer_' num2str(LayerSelected) '.mat']);
...
print(hb,['Figures/Similarity_' type '_layer_' num2str(LayerSelected) printpostfix],printmode,printoption);
...
pos = get(hb,'Position');
set(hb,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
print(hb,['Figures/SimilarityRatio_' type '_layer_' num2str(LayerSelected) printpostfix],printmode,printoption);
