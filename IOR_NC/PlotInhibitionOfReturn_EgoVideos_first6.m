...
if strcmp(type, 'os')
    imgsizeO = 1280;
    visualdeg=60;
    LongThres = 6; %for first 6 fixation analysis
    ShortThres = 6;
    ...
    if preprocess == 1
        ...
        save(['Mat/' type '_Fix.mat'],'Fix_posx','Fix_posy','Fix_time','Fix_pic','Fix_endIndex','Fix_microsad');
...
[propnt1, offsetnt1, propt1, offsett1, countpropntD, countpropntN, countproptD, countproptN] = fcn_MMevalInhibitionOfReturn_exp23only(Firstx, Firsty, centerRx, centerRy, IORthres, IORthresT, type, imgvalid, 1, type, LongThres, ShortThres);
save(['Mat/IOR_' type '_first6.mat'],'propnt1', 'offsetnt1', 'propt1', 'offsett1','countpropntD', 'countpropntN', 'countproptD', 'countproptN'); 

overallMeanS1 = []; overallStdS1 = [];
offsettotalNT = [];
load(['Mat/IOR_' type '_first6.mat']);
...
load(['Mat/' type '_chanceoffsetnt_first6.mat']);
[propt, propnt] = fcn_MMshuffleIOR(countpropntD, countpropntN, countproptD, countproptN);
...
print(hb,['Figures/IOR_first_overall_' type '_Deg_' num2str(Deg*100) '_first6' printpostfix],printmode,printoption);
