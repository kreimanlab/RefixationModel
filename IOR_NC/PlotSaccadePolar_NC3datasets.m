...
for T = 1:length(TYPELIST)
    ...
    load([wp 'Datasets/SubjectArray/' type '.mat']);
    load([wp 'Datasets/SubjectArray/' type '_seq.mat']);
    ...
    if 1
        ...
        for s = 1: length(subjlist)
            load([wp 'Datasets/ProcessScanpath_' type  '/' subjlist{s} '.mat']);
    ...
    if 1
        ...
        %caxis([0 0.5])
        caxis([0 max(Z(:))]);
        tt = title({[type ' : To-be-Revisited'];' ';' '},'fontsize',14,'interpreter','none');
...
save('Figures/fig1_turning_angle_human.mat','fig1_turning_angle');
