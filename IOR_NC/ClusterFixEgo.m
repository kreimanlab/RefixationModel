function [fixationstats] = ClusterFixEgo(eyedat,samprate)
...
for cndlop = 1:length(eyedat)
    if size(eyedat{cndlop},2) > 500/samprate/1000
        ...
        fixationtimes(:,(diff(fixationtimes,1) < MMDurationThres)) = []; %25 ms duration threshold
        ...
        tooshort = find(diff(saccadetimes,1) < MMSaccadeThres); %10 ms duration threshold for saccades
        notbehav = [];
        for ii = 1:length(tooshort);
        ...
        [fixationtimes] = BehavioralIndex(fixationindexes);
        fixationtimes(:,(diff(fixationtimes,1) < MMDurationThres))= []; %25 ms duration threshold for fixations
        fixationindexes = [];
        for ii = 1:size(fixationtimes,2);
