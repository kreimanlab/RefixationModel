...
for this_Deg = these_Degs
    ...
    for T = 1:length(TYPELIST)
        ...
        for su = 1:numel(all_subjlist)
            ...
            if strcmp(type,'naturalsaliency')
                ...
                if strcmp(type, 'array')
                    hb = figure('Visible','off','Position',[303   294   125+20   186+20]); hold on;
                else
                    hb = figure('Visible','off','Position',[303   294   125   186]); hold on;
                end
            ...
            if ~strcmp(type, 'naturalsaliency') && (~isempty(offsettotalT) && ~isempty(offsettotalNT))
    ...
    save(['Figures/fig3_fix_dva_human_' deg_str '.mat'],'fig3_fix_dva'); 
