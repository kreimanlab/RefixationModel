...
if strcmp(subtype, 'cmonkey')
    ...
    if preprocess == 1
        ...
        save(['Mat/' subtype '_Fix.mat'],'Fix_posx','Fix_posy','Fix_time','Fix_pic','Fix_microsad');
...
% [propnt1, offsetnt1, propt1, offsett1,countpropntD, countpropntN, countproptD, countproptN ] = fcn_MMevalInhibitionOfReturn3overlaps(Firstx, Firsty, centerRx, centerRy, IORthres, IORthresT, type, imgvalid, 1, type);
% save(['Mat/IOR_' type '_comb_3overlaps.mat'],'propnt1', 'offsetnt1', 'propt1', 'offsett1','countpropntD', 'countpropntN', 'countproptD', 'countproptN'); 

overallMeanS1 = []; overallStdS1 = [];
offsettotalNT = [];
load(['Mat/IOR_' type '_comb_3overlaps.mat']);
...
subtype = strsplit(type, '_'); 
load(['Mat/' subtype{1} '_chanceoffsetnt_3overlaps.mat']);
[propt, propnt] = fcn_MMshuffleIOR(countpropntD, countpropntN, countproptD, countproptN);
[h pnt] = ttest2(propnt,Mpropnt1); 
chanceval = mean(propnt);

if pnt <= 0.05
    text(1,9, '*', 'Fontsize', 12, 'Fontweight', 'bold');
% elseif pvalue <= 0.01
%     text(s-0.05,max(intersubj_consis_avg)+max(intersubj_consis_std)+0.05, '**', 'Fontsize', 16, 'Fontweight', 'bold');
% elseif pvalue <= 0.001
%     text(s-0.05,max(intersubj_consis_avg)+max(intersubj_consis_std)+0.05, '*', 'Fontsize', 16, 'Fontweight', 'bold');
else
    text(1,9, 'n.s.', 'Fontsize', 12, 'Fontweight', 'bold');
end
...
if strcmp(type, 'array')
    ...
    %plot([0:3],ones(size([0:3]))* chanceval*100,'k:','LineWidth',2.5);
else
    %plot([0:3],ones(size([0:3]))* IORthres*IORthres*pi/(596*596)*100,'k:','LineWidth',2.5);
    plot([0:3],ones(size([0:3]))* chanceval*100,'k:','LineWidth',2.5);
end
