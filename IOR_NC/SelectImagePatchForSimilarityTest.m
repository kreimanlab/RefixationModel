...
firstS = Selected(:,2);
[a b] = unique(firstS);
SelectedT = Selected(b,:);
save(['Mat/' type '_selected.mat'],'SelectedT');

load(['Mat/' type '_selected.mat']);
NsampleList = NsampleList(1:25);
WriteDir = ['/home/mengmi/Desktop/' type '/'];
for x = NsampleList
    s = SelectedT(x,1);
    i = SelectedT(x,2);
    j = SelectedT(x,3);
    ...
    imwrite(img, [WriteDir 'stimuli_' num2str(i) '_Return.jpg']);
	...
	img = imread(filename);
    img = imresize(img, [224 224]);
    imwrite(img, [WriteDir 'stimuli_' num2str(i) '_nonReturn.jpg']);
